...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    /* ───────────── ALL expenses (both individual and group) ───────────── */
    const allExpenses = (await ctx.db.query("expenses").collect()).filter(
      (e) =>
    ...
    /* ───────────── ALL settlements (both individual and group) ───────────── */
    const allSettlements = (await ctx.db.query("settlements").collect()).filter(
      (s) =>
